...
export const createOrderSchema = Joi.object({
  customer_name: Joi.string().required().min(1).max(255).messages({
    ...
    "string.min": "Customer name must be at least {{ #limit }} character",
    "string.max": "Customer name must not exceed {{ #limit }} characters",
    "any.required": "Customer name is required",
  }),
  product_name: Joi.string().required().min(1).max(255).messages({
    "string.empty": "Product name is required",
    "string.min": "Product name must be at least {{ #limit }} character",
...
export const updateOrderStatusSchema = Joi.object({
  ...
  .required()
  .messages({
    "any.required": "Order status data is required",
  });
